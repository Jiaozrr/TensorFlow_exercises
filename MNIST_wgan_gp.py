...
grad = tf.gradients(D(X_inter), [X_inter])[0]  # 对我们选取的这些值求偏导数
grad_norm = tf.sqrt(tf.reduce_sum((grad)**2, axis=1))  # 得到欧氏距离
grad_pen = 10 * tf.reduce_mean(tf.nn.relu(grad_norm - 1.))  # 取grad_norm和1中间更大的一个，再求平均
